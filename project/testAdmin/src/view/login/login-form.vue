...
export default {
  ...
  data () {
    const validateusername = (rule, value, callback) => {
      let _this=this
        if (value === '') {
            callback(new Error(_this.$t('header.login_inputUserErr')));
        }else{
          callback()
        }
    };
    const validatepass = (rule, value, callback) => {
      let _this=this
        if (value === '') {
            callback(new Error(_this.$t('header.login_inputPasswordErr')));
        }else{
          callback()
        }
    };
    const validatecaptcha = (rule, value, callback) => {
      let _this=this
        if (value === '') {
            callback(new Error(_this.$t('login_inputCaptchaErr')));
        }else{
          callback()
        }
    };
